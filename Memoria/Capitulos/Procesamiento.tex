...
imagenes y crear el canvas sobre el que guardar el resultado.
...
Con el fin de facilitar la comparación de los pixels la imágen pasa por un
...
imágen. Si la estrella no se encuentra en el centro es imposible para este
...
Al igual que la etapa definida en la seccion \ref{cap3:sec:recolor} se emplea
la libreria PIL. Esto no es lo único que comparten, el proceso al que se
someten las imagenes es muy similar.
...
Una vez cargada la ímágen se analiza cada pixel que la compone, para conocer
...
la imágen que pertenecen a dicha tonalidad.
...
En esta etapa empleamos la libreria image\_slicer de Python, distribuida bajo
licencia MIT. Esta libreria permiter dividir una fotografía en n piezas del
mismo tamaño.

\medskip

En este caso decidimos que las imagenes se dividieran en 9 sectores, lo cual
...
memoria, debido a esto solo la imágen de la figura \ref{cap3:fig:qcenter}
aparecería en la salida. Si esto quisiese modificarse tan solo habria que
...
\begin{figure}[t]
  ...
  \caption{Imagenes resultantes de la división\label{cap3:fig:crop}}
\end{figure}
