...
def process(file, dimages, dout, dout2):
    ...
    logger.debug(
        '{0} full recolor stored in {1}/{2}.png'.format(file, dout, file[:-4]))
    imtrans.save('{0}/{1}.png'.format(dout, file[:-4]), "PNG")
    # store the partial recolor results in dout2 directory
    logger.debug(
        '{0} red and blue recolors stored in {1}/{2}_f.png and {1}/{2}_l.png'.format(file, dout2, file[:-4]))
...
def loop_step(dimages, input, dout, dout2, stop_event):
    '''Recolors every jpg file listed in input taking the associated image from dimages, for each file creates a png file in dout.
    ...
    if not os.path.isdir(dimages):
        if show:
            logger.critical(
                'Image input directory, {0}, not found.'.format(dimages))
        return 0
...
if __name__ == '__main__':
    try:
        settings.init()
        run(settings.accepted, settings.dpics,
            settings.drecolor, settings.dprecolor, Event())
